...
| 场景 | 调整参数 | 文件 |
|---|---|---|
| 并发请求量大 | 增加 `--workers` 数量（≤ 2×CPU核数） | `docker-compose.prod.yml` backend command |
| SQL 执行慢 | 增加 celery_worker concurrency | `docker-compose.prod.yml` celery_worker command |
