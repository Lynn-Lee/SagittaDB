...
- SQL 工单提交页资源组自动解析
- 核心后台列表页统一表格体验
- 实例数据库/Schema 同步按连接用户真实可见范围全量对齐
- Oracle 普通 Schema 用户与高权限账号的同步范围差异
- 已被资源组关联的实例删除阻断与提示
...
- `admin / Admin@2024!`（默认密码会触发强制改密，请先完成改密再继续完整验证）
...
### TC-AUTH-025A 用户组统一筛选与导出

步骤：

1. 打开“系统管理 -> 用户组管理”。
2. 分别设置组标识 / 中文名、组长、上级组、关联资源组、状态中的任意 2 个以上筛选条件。
3. 观察筛选标签与表格结果。
4. 将导出范围设置为“当前筛选结果”，分别点击“导出 Excel / 导出 CSV”。
...
### TC-AUTH-025B 删除被资源组关联的实例会被明确阻止

步骤：

1. 在“资源组管理”中确认某实例已被某资源组关联。
2. 打开“实例管理”，对该实例点击删除并确认。
...
### TC-AUTH-025C 资源组编辑页不应残留已删除实例 ID

步骤：

1. 先移除资源组中的实例关联，再删除该实例。
2. 返回“资源组管理”，编辑对应资源组。
...
### TC-AUTH-033 实例数据库/Schema 同步全量对齐当前连接用户可见范围

步骤：

1. 使用高权限账号配置一个 Oracle 实例并执行一次“从实例自动同步”。
2. 确认实例下已同步出较多 Schema。
3. 将同一实例改成普通 Schema 用户账号。
4. 再次点击“从实例自动同步”。

预期：

- 同步后只保留当前连接用户现在真实可见的 Schema
- 之前由高权限账号同步进来的、当前已不可见的旧 Schema 会被自动移除

### TC-AUTH-034 Oracle 普通 Schema 用户只同步自身 Schema

步骤：

1. 使用普通 Oracle 用户（如 `ANE`）创建或编辑实例。
2. 点击“从实例自动同步”。

预期：

- 列表中只出现该普通用户当前真实可见的 Schema
- 不会再同步出大量系统 Schema 或其他业务 Schema
