...
- Dashboard 一期包含“在线查询概览”和“SQL 工单概览”两大模块
- Dashboard 两个模块均按当前用户权限范围裁剪数据，并支持统一时间范围筛选（`7/14/30/60` 天 + 自定义天数）
- Dashboard 在线查询模块包含 7 个卡片、查询趋势、查询用户 Top 10、治理趋势和单独的待审批库存趋势
- Dashboard SQL 工单模块包含 10 个卡片、工单提交趋势、工单治理趋势、执行趋势、单独的待审批库存趋势，以及工单提交用户 / 热点实例 / 热点数据库 / 审批人处理 / 执行实例 Top 10
