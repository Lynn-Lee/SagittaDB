...
export default function UserManagement() {
  ...
  const openEdit = (r: any) => { setEditId(r.id); form.setFieldsValue({ ...r, password: '' }); setModalOpen(true) }
  ...
  const columns: ColumnsType<any> = [
    ...
    { title: '角色', width: 120, render: (_, r) => r.is_superuser ? <Tag color="red">超级管理员</Tag> : <Tag>普通用户</Tag> },
    { title: '认证', dataIndex: 'auth_type', width: 90, render: t => <Tag>{t}</Tag> },
    { title: '2FA', dataIndex: 'totp_enabled', width: 80, render: v => v ? <Tag color="green">已开启</Tag> : <Tag>未开启</Tag> },
  ...
  return (
    <div>
      ...
      <Modal title={editId ? '编辑用户' : '新建用户'} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)}
        okText={editId ? '保存' : '创建'} confirmLoading={createMut.isPending || updateMut.isPending}>
