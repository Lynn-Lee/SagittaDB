        ...
        for fp, fp_rows in grouped.items():
            ...
            items.append(
                SlowQueryFingerprintItem(
                    ...
                    instance_count=len({row.instance_id for row in fp_rows if row.instance_id is not None}),
                    database_count=len({(row.instance_id, row.db_name) for row in fp_rows if row.db_name}),
                    count=len(fp_rows),
