...
class DashboardService:
    ...
    async def get_query_overview(db: AsyncSession, user: dict, days: int = 7) -> dict:
        ...
        period_query_user_count = await scalar(
            select(func.count(distinct(period_query_subq.c.user_id)))
        )
        ...
        apply_failure_subq = DashboardService._apply_query_apply_scope(
            select(QueryPrivilegeApply).where(
                and_(
                    QueryPrivilegeApply.updated_at >= period_start, QueryPrivilegeApply.status == 2
                )
            ),
            scope,
        ).subquery()
        ...
        approved_apply_subq = DashboardService._apply_query_apply_scope(
            select(QueryPrivilegeApply).where(
                and_(
                    QueryPrivilegeApply.updated_at >= period_start, QueryPrivilegeApply.status == 1
                )
            ),
            scope,
        ).subquery()
        approved_query_priv_apply_count = await scalar(
            select(func.count()).select_from(approved_apply_subq)
        )
        ...
        pending_query_priv_apply_count = await scalar(
            select(func.count()).select_from(pending_apply_subq)
        )
        ...
        revoked_query_privilege_count = await scalar(
            select(func.count()).select_from(revoked_priv_subq)
        )

        trend_stmt = (
            DashboardService._apply_query_scope(
                select(
        ...
        query_failure_stmt = DashboardService._apply_query_scope(
            ...
            ).where(and_(QueryLog.created_at >= period_start, QueryLog.priv_check.is_(False))),
            scope,
        ).group_by(func.date(QueryLog.created_at))
        ...
        masked_stmt = DashboardService._apply_query_scope(
            ...
            ).where(and_(QueryLog.created_at >= period_start, QueryLog.masking.is_(True))),
            scope,
        ).group_by(func.date(QueryLog.created_at))
        ...
        top_stmt = (
            DashboardService._apply_query_scope(
                select(
                    QueryLog.user_id,
                    func.count().label("query_count"),
                ).where(and_(QueryLog.created_at >= period_start, QueryLog.user_id.is_not(None))),
                scope,
            )
        ...
        if top_user_ids:
            user_rows = await db.execute(
                select(Users.id, Users.display_name, Users.username).where(
                    Users.id.in_(top_user_ids)
                )
            )
    ...
    async def get_instance_overview(db: AsyncSession, user: dict) -> dict:
        ...
        type_stmt = (
            DashboardService._apply_instance_scope(
                select(Instance.db_type, func.count().label("count")).where(
                    Instance.is_active.is_(True)
                ),
                scope,
            )
            .group_by(Instance.db_type)
            .order_by(func.count().desc(), Instance.db_type)
        )
        ...
        enabled_instance_count = await scalar(
            select(func.count()).select_from(active_instance_subq)
        )
        ...
        disabled_instance_count = await scalar(
            select(func.count()).select_from(disabled_instance_subq)
        )
        ...
        return {
            ...
            "instance_type_distribution": [
                {"db_type": row.db_type, "count": int(row.count or 0)} for row in type_rows
            ],
    ...
    async def _archive_conditions_for_user(db: AsyncSession, user: dict) -> list:
        if (
            ...
            return []
        visible_workflow_ids = await DashboardService._visible_archive_workflow_ids_for_user(
            db, user
        )
    ...
    async def get_archive_overview(db: AsyncSession, user: dict, days: int = 7) -> dict:
        ...
        pending_count = await scalar(
            select(func.count()).select_from(
                archive_stmt(ArchiveJob.status == ArchiveJobStatus.PENDING_REVIEW).subquery()
            )
        )
        approved_count = await scalar(
            select(func.count()).select_from(
                archive_stmt(ArchiveJob.status == ArchiveJobStatus.APPROVED).subquery()
            )
        )
        scheduled_count = await scalar(
            select(func.count()).select_from(
                archive_stmt(ArchiveJob.status == ArchiveJobStatus.SCHEDULED).subquery()
            )
        )
        running_count = await scalar(
            select(func.count()).select_from(
                archive_stmt(
        ...
        paused_count = await scalar(
            select(func.count()).select_from(
                archive_stmt(
                    ArchiveJob.status.in_((ArchiveJobStatus.PAUSING, ArchiveJobStatus.PAUSED))
                ).subquery()
            )
        )
        high_risk_active_count = await scalar(
            select(func.count()).select_from(
                archive_stmt(
        ...
        submit_trend_rows = (
            await db.execute(
                ...
                .group_by(func.date(ArchiveJob.created_at))
                .order_by(func.date(ArchiveJob.created_at))
            )
        ).all()
        submit_trend_map = {
            str(row.d): {
        ...
        finished_trend_rows = (
            await db.execute(
                ...
                .with_only_columns(
                    ...
                    func.sum(
                        case((ArchiveJob.status == ArchiveJobStatus.SUCCESS, 1), else_=0)
                    ).label("success_count"),
                    func.sum(
                        case((ArchiveJob.status == ArchiveJobStatus.FAILED, 1), else_=0)
                    ).label("failed_count"),
                    func.sum(
                        case((ArchiveJob.status == ArchiveJobStatus.CANCELED, 1), else_=0)
                    ).label("canceled_count"),
                    func.coalesce(func.sum(ArchiveJob.processed_rows), 0).label("processed_rows"),
                )
                .group_by(func.date(ArchiveJob.finished_at))
                .order_by(func.date(ArchiveJob.finished_at))
            )
        ).all()
        finished_trend_map = {
            str(row.d): {
        ...
        for offset in range(days):
            ...
            active_stock_count.append(
                await scalar(select(func.count()).select_from(active_stock_stmt))
            )
        ...
        top_submitter_rows = (
            await db.execute(
                ...
                .group_by(ArchiveJob.created_by_id)
                .order_by(func.count().desc())
                .limit(10)
            )
        ).all()
        submitter_ids = [
            row.created_by_id for row in top_submitter_rows if row.created_by_id is not None
        ]
        user_map: dict[int, str] = {}
        if submitter_ids:
            user_rows = await db.execute(
                select(Users.id, Users.display_name, Users.username).where(
                    Users.id.in_(submitter_ids)
                )
            )
            user_map = {
        ...
        top_table_rows = (
            await db.execute(
                ...
                .group_by(ArchiveJob.source_db, ArchiveJob.source_table)
                .order_by(func.sum(ArchiveJob.estimated_rows).desc())
                .limit(10)
            )
        ).all()
    ...
    async def get_workflow_overview(db: AsyncSession, user: dict, days: int = 7) -> dict:
        ...
        def workflow_stmt(*conditions):
            ...
            stmt = stmt.where(
                SqlWorkflow.id.in_(
                    select(WorkflowAudit.workflow_id).where(
                        WorkflowAudit.workflow_type == int(WorkflowType.SQL)
                    )
                )
            )
            if conditions:
        ...
        today_approved_count = await scalar(
            select(func.count(distinct(period_approved_subq.c.workflow_id))).select_from(
                period_approved_subq
            )
        )

        period_rejected_subq = (
            workflow_log_stmt(
                WorkflowLog.created_at >= period_start,
        ...
        today_rejected_count = await scalar(
            select(func.count(distinct(period_rejected_subq.c.workflow_id))).select_from(
                period_rejected_subq
            )
        )
        ...
        today_execute_success_count = await scalar(
            select(func.count()).select_from(execute_success_subq)
        )
        ...
        today_execute_failed_count = await scalar(
            select(func.count()).select_from(execute_failed_subq)
        )
        ...
        today_cancel_count = await scalar(
            select(func.count(distinct(cancel_subq.c.workflow_id))).select_from(cancel_subq)
        )
        ...
        submit_trend_stmt = (
            ...
            .group_by(func.date(SqlWorkflow.created_at))
            .order_by(func.date(SqlWorkflow.created_at))
        )
        ...
        approved_trend_map = {
            str(row.d): int(row.approved_count or 0) for row in approved_trend_rows
        }
        ...
        rejected_trend_map = {
            str(row.d): int(row.rejected_count or 0) for row in rejected_trend_rows
        }
        ...
        execute_success_trend_map = {
            str(row.d): int(row.success_count or 0) for row in execute_success_trend_rows
        }

        execute_failed_trend_stmt = (
            workflow_stmt(
                SqlWorkflow.finish_time >= period_start,
                SqlWorkflow.status == WorkflowStatus.EXCEPTION,
            )
        ...
        execute_failed_trend_map = {
            str(row.d): int(row.failed_count or 0) for row in execute_failed_trend_rows
        }
        ...
        for offset in range(days):
            ...
            queued_stock_count.append(
                await scalar(select(func.count()).select_from(queued_stock_stmt))
            )
            ...
            running_stock_count.append(
                await scalar(select(func.count()).select_from(running_stock_stmt))
            )
        ...
        top_submitter_stmt = (
            ...
            .group_by(SqlWorkflow.engineer_id)
            .order_by(func.count().desc())
            .limit(10)
        )
        top_submitter_rows = (await db.execute(top_submitter_stmt)).all()
        submitter_ids = [
            row.engineer_id for row in top_submitter_rows if row.engineer_id is not None
        ]
        user_map: dict[int, str] = {}
        if submitter_ids:
            user_rows = await db.execute(
        ...
        top_instance_stmt = (
            ...
            .join(Instance, SqlWorkflow.instance_id == Instance.id)
            .with_only_columns(
                Instance.instance_name,
                func.count().label("count"),
            )
        ...
        top_approver_stmt = (
            ...
            .group_by(WorkflowLog.operator_id)
            .order_by(func.count().desc())
            .limit(10)
        )
        ...
        return {
            ...
            "top_submitters": [
                {
                    "display_name": user_map.get(row.engineer_id, f"用户{row.engineer_id}"),
                    "count": int(row.count or 0),
                }
                for row in top_submitter_rows
                if row.engineer_id is not None
            ],
            "top_instances": [
            ...
            "top_approvers": [
                {
                    "display_name": user_map.get(row.operator_id, f"用户{row.operator_id}"),
                    "count": int(row.count or 0),
                }
                for row in top_approver_rows
    ...
    async def get_stats(db: AsyncSession, days: int = 30) -> dict:
        ...
        async def wf_count_by_status(status: int) -> int:
            return await count(
                select(func.count())
                .select_from(SqlWorkflow)
                .where(and_(SqlWorkflow.created_at >= period_start, SqlWorkflow.status == status))
            )
        ...
        return {
            ...
            "monitor_instance_total": await count(
                select(func.count())
                .select_from(MonitorCollectConfig)
                .where(MonitorCollectConfig.is_enabled)
            ),
        }
        ...
        for i in range(days - 1, -1, -1):
            ...
            async def day_count_status(status_val: int, ds=day_start, de=day_end) -> int:
                stmt = (
                    ...
                    .where(
                        and_(
                            SqlWorkflow.created_at >= ds,
                            SqlWorkflow.created_at < de,
            ...
            async def day_count_all(ds=day_start, de=day_end) -> int:
                stmt = (
                    select(func.count())
                    .select_from(SqlWorkflow)
                    .where(and_(SqlWorkflow.created_at >= ds, SqlWorkflow.created_at < de))
                )
                return (await db.execute(stmt)).scalar_one()
