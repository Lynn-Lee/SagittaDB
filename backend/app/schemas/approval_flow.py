...
class ApprovalFlowNodeCreate(BaseModel):
    ...
    approver_type: Literal["users", "group", "any_reviewer"] = Field(
...
class ApprovalFlowCreate(BaseModel):
    ...
    @classmethod
    def orders_unique_and_sequential(cls, v: list[ApprovalFlowNodeCreate]) -> list[ApprovalFlowNodeCreate]:
        if not v:
            return v
...
class ApprovalFlowUpdate(BaseModel):
    ...
    @field_validator("nodes")
    @classmethod
    def orders_valid(cls, v: list[ApprovalFlowNodeCreate] | None) -> list[ApprovalFlowNodeCreate] | None:
        if v is None:
            return v
        if not v:
            return v
